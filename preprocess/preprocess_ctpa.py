...
FILLIN_VALUE_SCAN = -1000  # Air Hounsfield Units, as placeholder for failed segmentation (TODO: A guess, ask Noa)
...
def dicom_get_pixels_hu(scans):
    ...
    # Set outside-of-scan pixels (DICOM placeholders are commonly set to -2000) to 0.
...
def preprocess_ctpa_directory(src_path, dst_path):
    ...
    for accession_number in os.listdir(src_path):#[int(len(os.listdir(src_path))/2):-1]:#[int(len(os.listdir(src_path))/2):]:#[::-1]:#[int(len(os.listdir(src_path))/2):-1]:
